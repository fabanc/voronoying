...
def validate_spatial_ref(feature_class, factor):
    """
    Validate that the input point and line feature classes share spatial reference
    :param: feature_class: input feature class to validate
    ...
    if sr.XYTolerance > tolerance:
        arcpy.AddWarning(f"The spatial tolerance for {name} is {sr.XYTolerance}. The factor your have specified matches a tolerance of {tolerance} or 1/{factor}. The output generated to validate your topology or generate Voronoi cells will be less fine grained than input data.")
    return sr
...
def validate_topology(inpoints: str, inlines: str, factor: int, out_gdb: str):
    ...
    try:
        ...
        srp = validate_spatial_ref(inpoints, factor)
        srl = validate_spatial_ref(inlines, factor)
        #wkt strings
        ...
        if not out_gdb.endswith('.gdb'):
            raise ValueError('invalid output gdb extension')
        parentdir = os.path.dirname(out_gdb)
        arcpy.management.CreateFileGDB(parentdir, os.path.basename(out_gdb))
        arcpy.env.workspace = out_gdb
        
        spatial_reference = srl
